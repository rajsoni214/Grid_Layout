...
import com.example.miniproject1.model.StatusModel;
...
public class StatusFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
       ...
       View v= inflater.inflate(R.layout.fragment_status, container, false);
        ...
        // get all status from database and bind into recylerview
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        {
            Random r=new Random();
            String  filename= FirebaseAuth.getInstance().getCurrentUser().getUid()+""+r.ints(10000000);
            FirebaseStorage.getInstance().getReference().child("status").child(filename)
                ...
                public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                   taskSnapshot.getStorage().getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                       ...
                       public void onSuccess(Uri uri) {
                           // now the status uploading is successful, let's upload atatus info into database
                           FirebaseDatabase.getInstance().getReference().child("users").child(FirebaseAuth.getInstance()
                               ...
                               public void onDataChange(@NonNull DataSnapshot snapshot) {
                                 String name=snapshot.child("name").getValue(String.class);
                                   ...
                                   FirebaseDatabase.getInstance().getReference().child("status").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child("images").push().setValue(images);
